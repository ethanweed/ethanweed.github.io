...
## Bar plots
There are many different ways these data could be represented graphically. A common way to look at data from different experimental conditions is to make a bar plot. This is the format that all ggplot commands will take. First, write ``ggplot`` followed by ``()``. Inside the parentheses, write the name of the dataframe (if you want, you can also write e.g. ``data = df_long``), but you don't have to. Then define "aesthetics" by writing ``aes`` followed by ``()``. Inside the parentheses, define the x and y axes. You don't have to write ``x =`` or ``y =``, but I think it makes it clearer. If you don't, R will assume that the first axis you write is x. As you will see later, there are other things you can add to the aesthetics. Next, write ``+`` and the kind of graph you want. These are called "geometries". To make a barplot, write ``geom_bar()``. The command ``stat = "identity"`` is needed for barplots, but ususally you won't need this. 
```{r}
